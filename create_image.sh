...
if qemu-img convert -c /os/${DISTR}.img -O qcow2 /os/${DISTR}.qcow2 2>/dev/null; then
    echo "Successfully converted to qcow2 format"
else
    echo "Warning: qcow2 conversion failed (likely due to emulation limitations)"
    echo "The raw .img file is still available and can be used directly"
    # Create a symbolic link or copy for compatibility
    cp /os/${DISTR}.img /os/${DISTR}.qcow2 2>/dev/null || true
fi
